...
export async function generateSundayWeeklyBrief(
  ...
  let prompt =
    'הפק הודעת פתיחת שבוע קשוחה, חדה וממוקדת לרמ"ד המבוססת על תמונת המצב החיה (משימות עומק, תג"בים השבוע, שחרורים ב-6 חודשים קרובים). עשה שימוש בעקרונות ה-doctrine ובדפוסים האישיים.\nהנחיית עיצוב: ההודעה בוואטסאפ צריכה להיות פשוטה ונקייה – כברירת מחדל אל תכלול מזהי משימות (ללא [מזהה X]) ואל תציין רמות עדיפות (ללא P1/P2/עדיפות), אלא אם כן הרמ"ד ביקש זאת במפורש. הצג את המשימות בפורמט נקי וקריא: כותרת המשימה, הצוות, והתג"ב בלבד.';
...
export async function generateDailyFocus(
  ...
  const context = systemContext ?? (await sheetsService.getSystemContext());
  let prompt =
    'הפק הודעת בוקר קצרה, נקייה וממוקדת לרמ"ד. שלוף 3 מיקודים דחופים להיום ובחר משרת/רש"צ אחד (מתוך הנתונים החיים) לשיחת 1-על-1 יזומה מהיום.\nהנחיית עיצוב: ההודעה בוואטסאפ צריכה להיות פשוטה ונקייה – כברירת מחדל אל תציין מזהה משימה (ללא [מזהה X]) ואל תציין עדיפות (ללא P1/P2/עדיפות), אלא אם כן הרמ"ד ביקש זאת במפורש. הצג את המשימות בפורמט נקי וקריא: כותרת המשימה, הצוות, והתג"ב בלבד.';
  if (userInstruction && userInstruction.trim()) {
    prompt += `\nדגש/הנחיה ספציפית מהרמ"ד להודעה זו: "${userInstruction.trim()}"`;
  }
  const agentResult = await geminiService.generateAgentResponse(prompt, [], context);
