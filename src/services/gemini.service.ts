...
`;
    ...
    const taskIntakeDirective = `
[כלל ברזל בהזנת משימות חדשות - אימות חובה של צוות ותאריך גמר ביצוע]:
1. כאשר הרמ"ד כותב או מבקש להוסיף משימה:
   - אם הרמ"ד לא ציין את שם הצוות (קסבה, טטריס, טקסס, ברוקלין, ארמורי, או רוחבי) - חובה עליך לעצור ולשאול אותו לאיזה צוות לשייך את המשימה.
   - אם הרמ"ד לא ציין תאריך גמר ביצוע (תג"ב) - חובה עליך לעצור ולשאול אותו מהו תאריך היעד/תג"ב.
   - אם חסרים גם צוות וגם תג"ב - שאל אותו ישירות על שניהם בהודעה אחת קצרה וממוקדת (למשל: "לאיזה צוות לשייך את המשימה ומה תאריך גמר הביצוע (תג"ב)?").
2. אסור באיסור מוחלט להמציא צוות, לנחש תאריך יעד, לבחור ברירת מחדל שרירותית, או לקרוא לכלי create_task עד שהצוות והתג"ב ברורים ומוגדרים.
3. רק לאחר שכל הפרטים (כותרת, צוות, תג"ב ועדיפות) קיימים וברורים - יש להפעיל את הכלי create_task.
`;
    ...
    const toolsConfig = [
      {
        functionDeclarations: [
          {
            name: 'create_task',
            description: 'יוצרת משימה חדשה ישירות בגיליון משימות_ותגב עם כותרת, צוות, תג"ב ועדיפות. יש להפעיל פונקציה זו רק אם הרמ"ד ציין במפורש את הצוות ואת תאריך גמר הביצוע (תג"ב). אם חסר צוות או תג"ב, אסור להפעיל כלי זה ויש לשאול את הרמ"ד תחילה!',
            parameters: {
              type: Type.OBJECT,
              properties: {
